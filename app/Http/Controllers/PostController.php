...
class PostController extends Controller
{
    ...
    public function update(Requesst $request, $id)
    {
        ...
        if($request->image)
        {
            $post->image = 'data:image/' . $request->image->extension() .';base64,' .base64_encode(file_get_contents($requeste->image));
        }

        $post->save();

        # 3. Delete all the records from the category_post related to the post
        ...
        #4. Save the new categories to categoriy_post table
        foreach($request->category as $category_id)
        {
            $category_post[] = ['category_id' => $category_id];
        } 
    ...
    public function delete($id)
    {
        ...
        return view('modals.delete')->with('post',$post);
    }
