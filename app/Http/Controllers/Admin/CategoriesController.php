...
class CategoriesController extends Controller
{
    ...
    public function index()
    {
        ...
        $uncategoriezed_count = 0;
        $all_posts = $this->post->all();
        foreach($all_posts as $post)
        {
            if($post->categoryPost->count() == 0)
            {
                $uncategoriezed_count++;
            }
        }
        return view('admin.categories.index')->with('all_categories', $all_categories)->with('uncategorized_count', $uncategoriezed_count);
    }
